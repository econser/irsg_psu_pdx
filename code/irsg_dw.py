...
def energy_batch_(image_dir, query, gmms, gt_bbox_dir=None, verbose=False, use_scaling=True):
    ...
    classes = [obj.names for obj in query.objects]
    filenames = os.listdir(os.path.join(image_dir, 'dog/'))
    results = []
    for file_num, filename in enumerate(filenames):
...
#def get_dw_gmms(model_filename='dw_gmms_n3_log.pkl'):
def get_dw_gmms(model_filename='dw_gmms_revised.pkl'):
    import irsg_utils as iu
    import cPickle
